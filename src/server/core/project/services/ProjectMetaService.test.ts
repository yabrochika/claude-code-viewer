...
describe("ProjectMetaService", () => {
  describe("getProjectMeta", () => {
    ...
    it("extracts project path from array-shaped raw json fallback", async () => {
      const program = Effect.gen(function* () {
        const storage = yield* ProjectMetaService;
        const projectId = Buffer.from("/test/project").toString("base64url");
        return yield* storage.getProjectMeta(projectId);
      ...
      const result = await Effect.runPromise(
        program.pipe(
          ...
          Effect.provide(
            testFileSystemLayer({
              readDirectory: () => Effect.succeed(["session1.jsonl"]),
              // Array-shaped line to cover non-object top-level json.
              readFileString: () =>
                Effect.succeed(
                  '[{"events":[{"meta":{"cwd":"/workspace/app-array"}}]}]',
                ),
              stat: () =>
                Effect.succeed({
                  type: "File",
      ...
      expect(result.projectName).toBe("app-array");
      expect(result.projectPath).toBe("/workspace/app-array");
      expect(result.sessionCount).toBe(1);
    });
  });
